...
//        Log.e(TAG, "dispatchTouchEvent: " + event.getActionIndex() + " " + event.getAction() + " " + event.getActionMasked() + " " + event.getPointerCount());
...
//        Log.e(TAG, "onTouchEvent: " + event.getActionIndex() + " " + event.getAction() + " " + event.getActionMasked() + " " + event.getPointerCount());
////        return super.onTouchEvent(event);
//        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
//            final ValueAnimator valueAnimator = ObjectAnimator.ofFloat(1, 10);
//            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
//                @Override
//                public void onAnimationUpdate(ValueAnimator animation) {
//                    final float value = (float) animation.getAnimatedValue();
//                    mRenderer.mScale = value;
//                    requestRender();
//                }
//            });
...
//            Log.e(TAG, "onTouchEvent: 点:" + i + " x:" + event.getX(i) + " rx:" + event.getRawX()
//                    + " y:" + event.getY(i) + " ry:" + event.getRawY());
//        }
//        updatePages();

//        if (mPageProvider != null) {
//            mPageProvider.onTouch(event);
//            updatePages();
//            requestRender();
//            return true;
//        }
        if (isCurl) {
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_DOWN://第一个手指按下
                    ...
                    mCurrentMode = Mode.DOWN;
                    mLastScale = mRenderer.mScale;
                    downEvent = MotionEvent.obtain(event);
                    checkDownEvent();
                    break;
                case MotionEvent.ACTION_POINTER_DOWN://第二个手指按下
                    mOldDistance = calculateDistance(event);
                    Log.e(TAG, "onTouchEvent: Down2:" + mOldDistance);
...
//                        Log.i(TAG, "onTouchEvent: old:" + mOldDistance + " new:" + newDistance);
//                        Log.i(TAG, "onTouchEvent: mLastScale:" + mLastScale + "  缩放:" + scale);
                        mRenderer.mScale = Math.max(1, mLastScale * scale);
                        mRenderer.mTranX = 0.2f;
                        mRenderer.mTranY = 0.2f;
                    }
                    if (mCurrentMode == Mode.DOWN) {
                        final float x = event.getX();
                        final float y = event.getY();

//                        mRenderer.mTranX += 0.1f;
//                        mRenderer.mTranY = 0f;
                    }
                    requestRender();
                    break;
...
//        return super.onTouchEvent(event);
    ...
    private void checkDownEvent() {
        if (mLastScale <= 1) {
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (mCurrentMode == Mode.DOWN && downEvent != null) {
                        isCurl = true;
                        onTouch(CurlView.this, downEvent);
                    }
                }
            }, 20);
        }
